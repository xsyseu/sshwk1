...
def get_IAT_features(path,hasher):
        ...
        try:
            for importeddll in pe.DIRECTORY_ENTRY_IMPORT:
                for importedapi in importeddll.imports:
                    IAT[str(importedapi.name)] = 1
        except:
            print("No DLL is imported in {0}".format(path))
    except:
        print("Error:Failed to open the file {0}".format(path))
    # hash the features using the hashing trick
    hashed_features = hasher.transform([IAT])
    ...
    print("Extracted {0} strings from {1}".format(len(IAT),path))
